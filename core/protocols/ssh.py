#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
Module SSH pour GhostCrack - Gère les connexions SSH
"""

import socket
import logging
import paramiko
from typing import Optional, Dict, Any

class SSHClient:
    """Client SSH pour brute force"""
    
    def __init__(self, host: str, username: str, password: str, 
                port: Optional[int] = None, timeout: int = 10, 
                proxy: Optional[str] = None):
        """
        Initialise un client SSH
        
        Args:
            host: Hôte cible
            username: Nom d'utilisateur
            password: Mot de passe à tester
            port: Port SSH (22 par défaut)
            timeout: Timeout en secondes
            proxy: Proxy à utiliser (format: socks5://127.0.0.1:9050)
        """
        self.host = host
        self.username = username
        self.password = password
        self.port = port or 22
        self.timeout = timeout
        self.proxy = proxy
        self.logger = logging.getLogger(__name__)
        self.last_response = {}
    
    def authenticate(self) -> bool:
        """
        Tente une authentification SSH
        
        Returns:
            bool: True si l'authentification réussit, False sinon
        """
        client = paramiko.SSHClient()
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        
        # Configuration du proxy si nécessaire
        sock = None
        if self.proxy:
            import socks
            proxy_type, proxy_addr = self._parse_proxy(self.proxy)
            if proxy_type and proxy_addr:
                self.logger.debug(f"Utilisation du proxy: {self.proxy}")
                
                # Création du socket proxy
                proxy_host, proxy_port = proxy_addr.split(':')
                sock = socks.socksocket()
                
                if proxy_type == 'socks5':
                    sock.set_proxy(socks.SOCKS5, proxy_host, int(proxy_port))
                elif proxy_type == 'socks4':
                    sock.set_proxy(socks.SOCKS4, proxy_host, int(proxy_port))
                elif proxy_type == 'http':
                    sock.set_proxy(socks.HTTP, proxy_host, int(proxy_port))
                
                sock.settimeout(self.timeout)
                sock.connect((self.host, self.port))
        
        try:
            client.connect(
                hostname=self.host,
                port=self.port,
                username=self.username,
                password=self.password,
                timeout=self.timeout,
                sock=sock,
                allow_agent=False,
                look_for_keys=False
            )
            
            # Si la connexion réussit, on récupère des informations
            transport = client.get_transport()
            server_id = transport.remote_version if transport else "Unknown"
            
            self.last_response = {
                'success': True,
                'server_id': server_id,
                'auth_method': 'password',
                'error': None
            }
            
            # On ferme proprement la connexion
            client.close()
            if sock:
                sock.close()
            
            return True
            
        except paramiko.AuthenticationException:
            self.last_response = {
                'success': False,
                'error': 'authentication_failed',
                'message': 'Authentication failed'
            }
            return False
            
        except paramiko.SSHException as e:
            # Détection des cas particuliers
            if "Too many authentication failures" in str(e):
                self.last_response = {
                    'success': False,
                    'error': 'rate_limited',
                    'message': str(e)
                }
            else:
                self.last_response = {
                    'success': False,
                    'error': 'ssh_error',
                    'message': str(e)
                }
            return False
            
        except (socket.timeout, socket.error) as e:
            self.last_response = {
                'success': False,
                'error': 'connection_error',
                'message': str(e)
            }
            return False
            
        except Exception as e:
            self.last_response = {
                'success': False,
                'error': 'unknown_error',
                'message': str(e)
            }
            return False
            
        finally:
            if client:
                client.close()
            if sock:
                sock.close()
    
    def _parse_proxy(self, proxy_str: str) -> tuple:
        """
        Parse une chaîne de proxy
        
        Args:
            proxy_str: Chaîne de proxy (ex: socks5://127.0.0.1:9050)
            
        Returns:
            tuple: (type, adresse) ou (None, None) si invalide
        """
        if not proxy_str or '://' not in proxy_str:
            return None, None
        
        try:
            proxy_type, proxy_addr = proxy_str.split('://')
            return proxy_type, proxy_addr
        except ValueError:
            return None, None
    
    def get_last_response(self) -> Dict[str, Any]:
        """
        Récupère la dernière réponse
        
        Returns:
            dict: La dernière réponse
        """
        return self.last_response